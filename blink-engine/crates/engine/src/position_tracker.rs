...
use crate::types::{Signal, OrderSide};
...
mod tests {
    ...
    fn test_no_hedge_same_side() {
        ...
        assert!(!tracker.is_hedge(&signal2), "Should not detect hedge (same side)");
    }
    ...
    fn test_no_hedge_different_size() {
        ...
        assert!(!tracker.is_hedge(&signal2), "Should not detect hedge (size mismatch)");
    }
}
