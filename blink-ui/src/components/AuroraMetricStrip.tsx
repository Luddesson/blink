...
export default function AuroraMetricStrip({
  ...
  const metrics: Metric[] = isLive ? [
    ...
    },
    {
      ...
      format: (v) => walletTruthVerified ? `${v >= 0 ? '+' : ''}$${fmt(v, 2)}` : 'unverified',
      icon: unrealized >= 0 ? <TrendingUp size={14} /> : <TrendingDown size={14} />,
      tone: walletTruthVerified ? (unrealized >= 0 ? 'bull' : 'bear') : 'neutral',
      subtitle: walletTruthVerified ? 'open positions' : 'truth required',
    },
    {
      label: 'Position Value',
      value: walletTruthVerified ? walletPositionValue : 0,
      format: (v) => walletTruthVerified ? `$${fmt(v, 2)}` : 'unverified',
      icon: <Activity size={14} />,
      tone: walletTruthVerified ? 'iris' : 'neutral',
    },
    {
    ...
    },
    {
      ...
      format: (v) => walletTruthVerified ? fmt(v, 0) : 'unverified',
      icon: <Target size={14} />,
