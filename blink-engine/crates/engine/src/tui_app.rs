...
use crate::activity_log::{ActivityEntry, ActivityLog, EntryKind, push as log_push};
...
use crate::paper_engine::{
    ExecutionSummary, ExperimentMetrics, ExperimentSwitches, FillWindowSnapshot, RejectionTrendPoint,
};
...
pub fn run_tui(
    ...
    let result = tui_loop(
        &mut term, portfolio, risk_status, book_store, activity, ws_live,
        trading_paused, rn1_wallet, markets, shutdown, start, msg_count,
        latency, kernel, risk_manager, fill_window, fill_latency,
        market_subscriptions, ws_force_reconnect, rejection_trend, execution_summary, experiment_data, experiment_switches,
        rn1_diagnostics,
        twin_snapshot,
        ws_health,
    );
    ...
    loop {
        ...
        if event::poll(Duration::from_millis(50))? {
            if let Event::Key(key) = event::read()? {
                if key.kind == KeyEventKind::Press {
                    if handle_key(key.code, key.modifiers, &mut state, &trading_paused,
                                  &activity, &shutdown, &risk_manager, &book_store, &markets,
                                  &market_subscriptions, &ws_force_reconnect, &experiment_switches) {
                        break;
                    }
                }
            }
        ...
        let perf = {
            ...
            let rn1_line = if rn1.consecutive_errors == 0 {
                Some(format!("rn1: OK polls={} sigs={}", rn1.total_polls, rn1.total_signals))
            } else {
                let status = rn1.last_http_status.map(|s| s.to_string()).unwrap_or_else(|| "n/a".to_string());
            ...
            let (ws_reconnects, ws_pongs) = if let Some(ref hm) = ws_health {
                (hm.reconnect_attempts.load(Ordering::Relaxed), hm.pong_recv.load(Ordering::Relaxed))
            } else {
                (0, 0)
            };
            PerfSnapshot {
        ...
        let snap = rt.block_on(async {
            ...
            let token_ids: Vec<String> = p.positions.iter().map(|pos| pos.token_id.clone()).collect();
        ...
        let kernel_snap = kernel.as_ref().map(|k| k.lock().unwrap().clone())
            .unwrap_or_default();
        let fill_window_snap = {
            fill_window.lock().unwrap().clone()
        };
        ...
        let subscribed_markets = {
            market_subscriptions.lock().unwrap().clone()
        };
        let twin_ui = {
            let t = rt.block_on(async { twin_snapshot.lock().await.clone() });
            TwinUiSnapshot {
                enabled: t.generation > 0 || t.total_signals > 0 || t.closed_trades > 0 || t.open_positions > 0,
        ...
        if kernel_snap.available {
            if kernel_snap.rtt.p99_us > RTT_ALERT_US && kernel_snap.rtt.samples > 10 {
                state.push_notification(NotificationKind::Warning,
                    format!("TCP RTT p99 {}us > {}us threshold", kernel_snap.rtt.p99_us, RTT_ALERT_US));
            }
            if kernel_snap.syscall.send_avg_us > SYSCALL_ALERT_US {
                state.push_notification(NotificationKind::Warning,
                    format!("Syscall send {}us > {}us threshold", kernel_snap.syscall.send_avg_us, SYSCALL_ALERT_US));
            }
        }
        ...
        // ── Render ────────────────────────────────────────────────────────
                term.draw(|f| {
                    render(f, &snap, &activity_snap, &rn1_wallet, &markets,
                        ws_connected, paused, uptime, &state, &perf, &kernel_snap,
                        &risk_snap, &book_store, &fill_window_snap, &fill_latency_samples,
                        &subscribed_markets, &twin_ui);
                })?;
...
fn handle_key(
    ...
    if state.config_editing {
        match code {
            ...
            KeyCode::Backspace => { state.config_edit_buf.pop(); }
            KeyCode::Char(c) => { state.config_edit_buf.push(c); }
            _ => {}
        }
        return false;
    }
    ...
    if state.market_search_active {
        match code {
            ...
            KeyCode::Backspace => { state.market_search.pop(); }
            KeyCode::Char(' ') => { state.market_search_active = false; }
            KeyCode::Char(c) => { state.market_search.push(c); }
            KeyCode::Enter => { state.market_search_active = false; }
            KeyCode::Down => {
    ...
    match code {
        ...
        KeyCode::Char('p') | KeyCode::Char('P') => {
            ...
            let msg = if was_paused { "Trading RESUMED" } else { "Trading PAUSED" };
        ...
        KeyCode::BackTab => {
            state.active_tab = if state.active_tab == 0 { TAB_COUNT - 1 } else { state.active_tab - 1 };
        }
        // Tab-specific keys
        _ => match state.active_tab {
            ...
            TAB_CONFIG    => handle_config_key(code, state, risk_manager, activity, experiment_switches),
            TAB_PERFORMANCE => {}
            TAB_TWIN => {}
            _ => {}
        }
    ...
    match code {
        ...
        KeyCode::Char(' ') => {
            ...
            let filtered: Vec<&String> = if state.market_search.is_empty() {
                ...
                all_markets.iter().filter(|m| m.to_lowercase().contains(&q)).collect()
            };
            let idx = state.selected_market_idx.min(filtered.len().saturating_sub(1));
            ...
            if let Some(pos) = subs.iter().position(|m| m == &token) {
                subs.remove(pos);
                log_push(activity, EntryKind::Engine, format!("Market unsubscribed: {}", shorten_token(&token, 16)));
            } else {
                subs.push(token.clone());
                log_push(activity, EntryKind::Engine, format!("Market subscribed: {}", shorten_token(&token, 16)));
            }
    ...
    match code {
        ...
        KeyCode::Enter | KeyCode::Char('e') => {
            ...
            state.config_edit_buf = match state.config_selected {
                ...
                4 => if cfg.trading_enabled { "true".into() } else { "false".into() },
        ...
        KeyCode::Char('r') => {
            ...
            if rm.is_circuit_breaker_tripped() {
                ...
                drop(rm);
                log_push(activity, EntryKind::Engine, "Circuit breaker RESET by operator".to_string());
            }
        }
        KeyCode::Char('x') | KeyCode::Char('X') => {
            let mut ex = experiment_switches.lock().unwrap();
            ex.sizing_variant_b = !ex.sizing_variant_b;
            log_push(activity, EntryKind::Engine, format!("Experiment sizing B={}", ex.sizing_variant_b));
        }
        KeyCode::Char('c') | KeyCode::Char('C') => {
            let mut ex = experiment_switches.lock().unwrap();
            ex.autoclaim_variant_b = !ex.autoclaim_variant_b;
            log_push(activity, EntryKind::Engine, format!("Experiment autoclaim B={}", ex.autoclaim_variant_b));
        }
        KeyCode::Char('v') | KeyCode::Char('V') => {
            let mut ex = experiment_switches.lock().unwrap();
            ex.drift_variant_b = !ex.drift_variant_b;
            log_push(activity, EntryKind::Engine, format!("Experiment drift B={}", ex.drift_variant_b));
        }
        _ => {}
    }
}
...
fn apply_config_edit(
    ...
    drop(rm);
    log_push(activity, EntryKind::Engine, format!("{label} updated to {val}"));
    state.push_notification(NotificationKind::Success, format!("{label} -> {val}"));
}

// ─── State snapshot ──────────────────────────────────────────────────────────

fn snapshot_portfolio(p: &PaperPortfolio, books: &OrderBookStore, risk_status: String) -> PortfolioSnapshot {
    let positions = p.positions.iter().map(|pos| {
        let current = books
        ...
        let upnl_pct = if pos.usdc_spent > 0.0 { upnl / pos.usdc_spent * 100.0 } else { 0.0 };
        PositionSnap {
            id:             pos.id,
    ...
    let closed_trades = p.closed_trades.iter().map(|t| {
        ClosedTradeSnap {
...
fn render(
    ...
    render_header(f, outer[0], snap, rn1_wallet, ws_live, paused, uptime_s, perf, state.modern_theme);
    ...
    match state.active_tab {
        TAB_DASHBOARD => render_dashboard(f, outer[2], snap, activity, markets,
                                          state.log_scroll, perf, kernel, state,
                                          fill_latency_samples),
        TAB_MARKETS   => render_markets_tab(f, outer[2], book_store, &state.market_search,
                                            state.selected_market_idx, markets, subscribed_markets),
        TAB_HISTORY   => render_history_tab(f, outer[2], snap, state.history_scroll, state),
        TAB_CONFIG    => render_config_tab(f, outer[2], risk, &snap.positions,
                                           state.config_selected, state.config_editing),
        TAB_PERFORMANCE => render_performance_tab(f, outer[2], snap, perf, kernel, state, fill_latency_samples),
        TAB_TWIN => render_twin_tab(f, outer[2], twin),
        _ => {}
    }

    render_hint(f, outer[3], state.active_tab, state.market_search_active, state.modern_theme);
...
fn render_modern(
    ...
    render_header(f, outer[0], snap, rn1_wallet, ws_live, paused, uptime_s, perf, true);
    ...
    let main_block = Block::default()
        ...
        .title(Span::styled(" WORKSPACE ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)));
    let main_inner = main_block.inner(body[0]);
    f.render_widget(main_block, body[0]);

    match state.active_tab {
        TAB_DASHBOARD => render_dashboard(f, main_inner, snap, activity, markets, state.log_scroll, perf, kernel, state, fill_latency_samples),
        TAB_MARKETS => render_markets_tab(f, main_inner, book_store, &state.market_search, state.selected_market_idx, markets, subscribed_markets),
        TAB_HISTORY => render_history_tab(f, main_inner, snap, state.history_scroll, state),
        TAB_CONFIG => render_config_tab(f, main_inner, risk, &snap.positions, state.config_selected, state.config_editing),
        TAB_PERFORMANCE => render_performance_tab(f, main_inner, snap, perf, kernel, state, fill_latency_samples),
        TAB_TWIN => render_twin_tab(f, main_inner, twin),
        _ => {}
    }

    render_modern_sidepanel(f, body[1], snap, ws_live, paused, perf, twin);
    render_hint(f, outer[3], state.active_tab, state.market_search_active, true);
    render_notifications(f, area, &state.notifications);
    render_fill_window_overlay(f, area, fill_window);
}
...
fn render_modern_sidepanel(
    ...
    let nav_card = vec![
        Line::from(Span::styled(" NET ASSET VALUE", Style::default().fg(Color::Gray))),
        Line::from(Span::styled(format!(" ${:.2}", snap.nav), Style::default().fg(pnl_color(nav_delta)).add_modifier(Modifier::BOLD))),
        Line::from(Span::styled(format!(" {:+.2}% session", nav_pct), Style::default().fg(pnl_color(nav_delta)))),
    ];
    f.render_widget(
        Paragraph::new(nav_card).block(
            Block::default()
                ...
                .title(Span::styled(" PORTFOLIO ", Style::default().fg(MONO_GREEN).add_modifier(Modifier::BOLD))),
        ),
        cols[0],
    );

    let attempts = snap.filled_orders + snap.aborted_orders + snap.skipped_orders;
    let fill_rate = if attempts > 0 { (snap.filled_orders as f64 / attempts as f64) * 100.0 } else { 0.0 };
    let exec_card = vec![
        Line::from(format!(" fills: {}  aborts: {}", snap.filled_orders, snap.aborted_orders)),
        Line::from(format!(" skips: {}  rate: {:.1}%", snap.skipped_orders, fill_rate)),
    ...
    f.render_widget(
        Paragraph::new(exec_card).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Rgb(65, 68, 78)))
                .title(Span::styled(" EXECUTION ", Style::default().fg(MONO_BLUE).add_modifier(Modifier::BOLD))),
        ),
        cols[1],
    );
    ...
    f.render_widget(
        Paragraph::new(status_card).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Rgb(65, 68, 78)))
                .title(Span::styled(" STATUS ", Style::default().fg(MONO_GOLD).add_modifier(Modifier::BOLD))),
        ),
        cols[2],
    );

    let spark_data: Vec<u64> = if snap.equity_curve.len() >= 2 {
        let min_val = snap.equity_curve.iter().cloned().fold(f64::INFINITY, f64::min);
        let max_val = snap.equity_curve.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
        ...
        Sparkline::default()
            ...
            .block(
                Block::default()
                    .borders(Borders::ALL)
                    .border_style(Style::default().fg(Color::Rgb(65, 68, 78)))
                    .title(Span::styled(" EQUITY TREND ", Style::default().fg(MONO_GREEN).add_modifier(Modifier::BOLD))),
            ),
        cols[3],
    );
}
...
fn render_tab_bar(f: &mut Frame, area: Rect, active: usize, modern_theme: bool) {
    let tabs = ["[1] Dashboard", "[2] Markets", "[3] History", "[4] Config", "[5] Performance", "[6] Blink Twin"];
    let spans: Vec<Span> = tabs.iter().enumerate().map(|(i, &label)| {
        if i == active {
            Span::styled(
                format!(" {label} "),
                if modern_theme {
                    Style::default().fg(Color::Black).bg(MONO_GREEN).add_modifier(Modifier::BOLD)
                } else {
...
fn render_header(
    ...
    let theme_chip_style = if modern_theme {
        Style::default().fg(Color::Black).bg(MONO_BLUE).add_modifier(Modifier::BOLD)
    } else {
    ...
    let main_line = Line::from(vec![
        ...
        Span::styled(format!("rc:{} ", perf.ws_reconnects), Style::default().fg(if perf.ws_reconnects > 20 { MONO_PINK } else { MONO_GRAY })),
        Span::styled(" │ ", Style::default().fg(MONO_GRAY)),
        Span::styled(
            perf.rn1_diag.clone().unwrap_or_else(|| "rn1: n/a".to_string()),
            Style::default().fg(if perf.rn1_diag.as_deref().unwrap_or("").contains("ERR") { MONO_PINK } else { MONO_GREEN }),
        ),
        Span::styled(" │ ", Style::default().fg(MONO_GRAY)),
...
fn render_portfolio_hud(f: &mut Frame, area: Rect, snap: &PortfolioSnapshot) {
    ...
    let stats_lines = vec![
        ...
        Line::from(vec![Span::styled("  UNREAL    ", label_style), Span::styled(format!("{:>+.2} USDC", snap.unrealized_pnl), Style::default().fg(pnl_color(snap.unrealized_pnl)).add_modifier(Modifier::BOLD))]),
        Line::from(vec![Span::styled("  REALIZED  ", label_style), Span::styled(format!("{:>+.2} USDC", snap.realized_pnl), Style::default().fg(pnl_color(snap.realized_pnl)).add_modifier(Modifier::BOLD))]),
        Line::from(vec![Span::styled("  SIGNALS   ", label_style), Span::styled(format!("{}", snap.total_signals), Style::default().fg(MONO_BLUE))]),
        Line::from(vec![Span::styled("  FILLED    ", label_style), Span::styled(format!("{}", snap.filled_orders), Style::default().fg(MONO_GREEN))]),
    ];
    ...
    let pct_table = vec![
        Line::from(vec![
            ...
            Span::styled(format!("{:+.2}%", nav_pct), Style::default().fg(pnl_color(nav_pct))),
        ]),
        Line::from(vec![
            Span::styled(" UNR   ", label_style),
            Span::styled(format!("{:+.2}%", unreal_pct), Style::default().fg(pnl_color(unreal_pct))),
        ]),
        Line::from(vec![
            Span::styled(" REAL  ", label_style),
            Span::styled(format!("{:+.2}%", realized_pct), Style::default().fg(pnl_color(realized_pct))),
        ]),
        Line::from(vec![
            Span::styled(" WIN   ", label_style),
            Span::styled(
                if snap.total_signals > 0 {
                    format!("{:.1}%", (snap.filled_orders as f64 / snap.total_signals as f64) * 100.0)
                } else {
                    "0.0%".to_string()
                },
        ...
        let canvas = Canvas::default()
            ...
            .paint(|ctx| {
                ...
                for r in [0.2_f64, 0.4, 0.6, 0.8] {
                    let gy = y0 + (y1 - y0) * r;
                    ctx.draw(&CanvasLine { x1: 0.0, y1: gy, x2: x1, y2: gy, color: grid_color });
                }
                for r in [0.25_f64, 0.5, 0.75] {
                    let gx = x1 * r;
                    ctx.draw(&CanvasLine { x1: gx, y1: y0, x2: gx, y2: y1, color: grid_color });
                }
...
fn render_positions_hud(f: &mut Frame, area: Rect, snap: &PortfolioSnapshot) {
    ...
    let rows: Vec<Row> = if snap.positions.is_empty() {
        vec![Row::new(vec![Cell::from("  NO ACTIVE EXPOSURE").style(Style::default().fg(MONO_GRAY))])]
    } else {
        snap.positions.iter().map(|pos| {
            let side_color = match pos.side {
                OrderSide::Buy  => MONO_GREEN,
                OrderSide::Sell => MONO_PINK,
            };
            let pnl_sty = Style::default().fg(pnl_color(pos.unrealized_pnl)).add_modifier(Modifier::BOLD);
            ...
            Row::new(vec![
                Cell::from(format!("{}", pos.side)).style(Style::default().fg(side_color).add_modifier(Modifier::BOLD)),
                Cell::from(
                    pos.market_title
                        .clone()
                ...
                Cell::from(
                    pos.market_outcome.clone().unwrap_or_else(|| "-".to_string())
                ).style(Style::default().fg(MONO_GOLD)),
                Cell::from(format!("{:.3}", pos.current_price)).style(Style::default().fg(Color::White)),
                Cell::from(format!("${:.0}", pos.usdc_spent)).style(Style::default().fg(MONO_GOLD)),
    ...
    .header(header)
    .column_spacing(2)
    .block(Block::default().title(" ACTIVE POSITIONS ").border_style(Style::default().fg(MONO_GRAY)).borders(Borders::LEFT));
...
fn render_kernel_hud(f: &mut Frame, area: Rect, kernel: &KernelSnapshot) {
    ...
    let hud_style = |val: u64, threshold: u64| {
        if val > threshold { MONO_PINK } else { MONO_GREEN }
    };

    let rtt_line = Line::from(vec![
        Span::styled("  RTT p99 ", Style::default().fg(MONO_GRAY)),
        Span::styled(format!("{}us", kernel.rtt.p99_us), Style::default().fg(hud_style(kernel.rtt.p99_us, 5000))),
    ]);
    let sched_line = Line::from(vec![
        Span::styled("  SCHED p99 ", Style::default().fg(MONO_GRAY)),
        Span::styled(format!("{}us", kernel.sched.p99_us), Style::default().fg(hud_style(kernel.sched.p99_us, 200))),
    ]);
    let sys_line = Line::from(vec![
        Span::styled("  SYSCALL send ", Style::default().fg(MONO_GRAY)),
        Span::styled(format!("{}us", kernel.syscall.send_avg_us), Style::default().fg(hud_style(kernel.syscall.send_avg_us, 100))),
    ]);

    f.render_widget(Paragraph::new(rtt_line).block(Block::default().borders(Borders::LEFT).border_style(Style::default().fg(MONO_GRAY))), cols[0]);
    f.render_widget(Paragraph::new(sched_line).block(Block::default().borders(Borders::LEFT).border_style(Style::default().fg(MONO_GRAY))), cols[1]);
    f.render_widget(Paragraph::new(sys_line).block(Block::default().borders(Borders::LEFT).border_style(Style::default().fg(MONO_GRAY))), cols[2]);
}

fn render_activity_hud(f: &mut Frame, area: Rect, entries: &[ActivityEntry], scroll: usize) {
    let inner_h = area.height as usize;
    let lines: Vec<Line> = entries.iter().rev().skip(scroll).take(inner_h).map(|e| {
        ...
        Line::from(vec![
            ...
            Span::styled(format!(" {:<6} ", match e.kind {
    ...
    f.render_widget(Paragraph::new(lines).block(Block::default().borders(Borders::TOP).border_style(Style::default().fg(MONO_GRAY))), area);
}
...
fn render_kernel_telemetry(f: &mut Frame, area: Rect, kernel: &KernelSnapshot, alert: bool) {
    ...
    let border_mod   = if alert { Modifier::BOLD | Modifier::SLOW_BLINK } else { Modifier::empty() };

    let block = Block::default()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(border_color).add_modifier(border_mod))
        .title(Span::styled(
            if alert { " !! KERNEL TELEMETRY !! " } else { " KERNEL TELEMETRY " },
    ...
    if !kernel.available {
        let na_line = Line::from(vec![
            ...
            Span::styled("(Windows/macOS or Linux build without ebpf-telemetry feature)", Style::default().fg(Color::DarkGray)),
        ]);
    ...
    // RTT column
    let rtt = &kernel.rtt;
    let rtt_val_style = if rtt.p99_us > RTT_ALERT_US { warn_style } else { val_style };
    let rtt_lines = vec![
        Line::from(vec![
            Span::styled(" TCP RTT ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
    ...
    let violations_style = if sched.threshold_violations > 0 { warn_style } else { val_style };
    let sched_lines = vec![
        Line::from(vec![
            Span::styled(" SCHED ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
    ...
    let sys_val_style = if sys.send_avg_us > SYSCALL_ALERT_US { warn_style } else { val_style };
    let sys_lines = vec![
        Line::from(vec![
            Span::styled(" SYSCALL ", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
...
fn render_fill_latency_histogram(f: &mut Frame, area: Rect, samples: &[u64]) {
    ...
    let lines: Vec<Line> = bins.iter().enumerate().map(|(i, &count)| {
...
fn render_fill_window_overlay(
    ...
    let lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(drift, Style::default().fg(drift_color).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
...
fn render_markets_tab(
    ...
    let search_title = if market_search.is_empty() {
        format!(" MARKETS [/] search [Space] toggle | active={} ", subscribed_markets.len())
    } else {
        format!(" MARKETS [{}] [Space] toggle | active={} ", market_search, subscribed_markets.len())
    };

    let list_lines: Vec<Line> = filtered.iter().enumerate().map(|(i, tid)| {
        let short = shorten_token(tid, 32);
        let mid = book_store.get_mid_price(tid)
        ...
        if i == selected_idx {
            ...
            Line::from(vec![
                Span::styled(format!("   {marker} {short}"), Style::default().fg(Color::White)),
    ...
    let list_block = Block::default()
        ...
        .title(Span::styled(search_title, Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)));
    let list_para = Paragraph::new(list_lines).block(list_block).wrap(Wrap { trim: true });
    ...
    let depth_block = Block::default()
        ...
        .title(Span::styled(
            format!(" ORDER BOOK DEPTH {} ",
                selected_token.map(|t| shorten_token(t, 16)).unwrap_or_default()),
    ...
    if let Some(book) = book {
        ...
        let bid_header = Row::new(vec![
            Cell::from("Bid Price").style(Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            Cell::from("Size").style(Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
        ]);
        let bid_rows: Vec<Row> = book.bids.iter().rev().take(15).map(|(&price, &size)| {
            Row::new(vec![
                Cell::from(format!("{:.3}", price as f64 / 1000.0)).style(Style::default().fg(Color::Green)),
                Cell::from(format!("{:.1}", size as f64 / 1000.0)).style(Style::default().fg(Color::White)),
            ])
        }).collect();

        let bid_table = Table::new(bid_rows, [Constraint::Percentage(50), Constraint::Percentage(50)])
            .header(bid_header)
        ...
        let ask_header = Row::new(vec![
            Cell::from("Ask Price").style(Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
            Cell::from("Size").style(Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
        ]);
        let ask_rows: Vec<Row> = book.asks.iter().take(15).map(|(&price, &size)| {
            Row::new(vec![
                Cell::from(format!("{:.3}", price as f64 / 1000.0)).style(Style::default().fg(Color::Red)),
                Cell::from(format!("{:.1}", size as f64 / 1000.0)).style(Style::default().fg(Color::White)),
            ])
        }).collect();

        let ask_table = Table::new(ask_rows, [Constraint::Percentage(50), Constraint::Percentage(50)])
            .header(ask_header);
...
fn render_history_tab(
    ...
    let outer = Layout::default()
        .direction(Direction::Vertical)
        .constraints([Constraint::Percentage(52), Constraint::Percentage(24), Constraint::Percentage(24)])
        .split(area);
    ...
    let rows: Vec<Row> = if snap.closed_trades.is_empty() {
        vec![Row::new(vec![Cell::from("  No closed trades yet").style(Style::default().fg(Color::DarkGray))])]
    } else {
        snap.closed_trades.iter().rev().skip(scroll).take(inner_h).map(|t| {
            let side_style = match t.side {
                OrderSide::Buy  => Style::default().fg(Color::Green).add_modifier(Modifier::BOLD),
                OrderSide::Sell => Style::default().fg(Color::Red).add_modifier(Modifier::BOLD),
            };
            let pnl_sty = Style::default().fg(pnl_color(t.realized_pnl)).add_modifier(Modifier::BOLD);
            Row::new(vec![
                Cell::from(shorten_token(&t.token_id, 12)).style(Style::default().fg(Color::White)),
                Cell::from(format!("{}", t.side)).style(side_style),
                Cell::from(format!("{:.3}", t.entry_price)).style(Style::default().fg(Color::DarkGray)),
                Cell::from(format!("{:.3}", t.exit_price)).style(Style::default().fg(Color::White)),
                Cell::from(format!("{:.2}", t.shares)),
                Cell::from(format!("{:>+.4}", t.realized_pnl)).style(pnl_sty),
                Cell::from(human_close_reason(&t.reason)).style(Style::default().fg(Color::DarkGray)),
                Cell::from(t.opened_at.clone()).style(Style::default().fg(Color::DarkGray)),
                Cell::from(format_age(t.duration_secs)).style(Style::default().fg(Color::DarkGray)),
            ])
        }).collect()
    };
    ...
    let trade_table = Table::new(rows, trade_widths)
        .header(header)
        .block(
            Block::default()
                .borders(Borders::ALL)
    ...
    let left_lines = vec![
        ...
        Line::from(format!("  Avg queue delay: {:.1} ms", delay_sum / trade_count)),
        Line::from(format!("  Trades scored: {}", snap.closed_trades.len())),
        Line::from(format!("  Shadow realism gap: {:.2} bps", state.execution_summary.shadow_realism_gap_bps)),
    ];
    f.render_widget(
        Paragraph::new(left_lines).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Magenta))
                .title(Span::styled(" EXECUTION SCORECARD ", Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD))),
        ),
        score_layout[0],
    );
    ...
    let right_lines: Vec<Line> = if tags.is_empty() {
        ...
        tags.into_iter().take(8).map(|(k, v)| Line::from(format!("  {:<18} {}", k, v))).collect()
    };
    f.render_widget(
        Paragraph::new(right_lines).block(
            Block::default()
                ...
                .title(Span::styled(" OUTCOME TAGS ", Style::default().fg(Color::Blue).add_modifier(Modifier::BOLD))),
        ),
        score_layout[1],
    );
    ...
    f.render_widget(
        Paragraph::new(exp_lines).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Cyan))
                .title(Span::styled(" A/B METRICS ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))),
        ),
        exp_rect,
    );
    ...
    for t in &snap.closed_trades {
        *pnl_by_token.entry(shorten_token(&t.token_id, 16)).or_insert(0.0) += t.realized_pnl;
    }
    ...
    let pnl_lines: Vec<Line> = pnl_list.iter().map(|(tok, pnl)| {
        Line::from(vec![
    ...
    let pnl_block = Block::default()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(Color::Blue))
        .title(Span::styled(" PnL ATTRIBUTION ", Style::default().fg(Color::Blue).add_modifier(Modifier::BOLD)));
    let pnl_para = Paragraph::new(pnl_lines).block(pnl_block).wrap(Wrap { trim: true });
    ...
    let dd_block = Block::default()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(Color::Magenta))
        .title(Span::styled(" DRAWDOWN TRACKER ", Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD)));
    ...
    let dd_lines = vec![
        Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default().fg(if snap.max_drawdown_pct > 5.0 { Color::Red } else { Color::Yellow }).add_modifier(Modifier::BOLD),
            ),
        ]),
        Line::from(vec![
    ...
    if snap.equity_curve.len() >= 2 {
        ...
        let spark_data: Vec<u64> = snap.equity_curve.iter().map(|&v| {
            if range < 0.001 { 50 }
            else { ((v - min_val) / range * 99.0) as u64 + 1 }
        }).collect();
        ...
        let spark = Sparkline::default()
            ...
            .block(Block::default()
                ...
                .title(Span::styled(" equity ", Style::default().fg(Color::DarkGray))));
...
fn render_config_tab(
    ...
    let params = [
        ("Max Daily Loss", format!("{:.1}%", risk.max_daily_loss_pct * 100.0)),
        ("Max Positions", format!("{}", risk.max_concurrent_positions)),
        ("Max Order Size", format!("${:.0}", risk.max_single_order_usdc)),
        ("Max Orders/sec", format!("{}", risk.max_orders_per_second)),
        ("Trading Enabled", if risk.trading_enabled { "YES".into() } else { "NO".into() }),
        ("VaR Threshold", format!("{:.1}%", risk.var_threshold_pct * 100.0)),
    ];

    let form_lines: Vec<Line> = params.iter().enumerate().map(|(i, (label, value))| {
    ...
    let form_para = Paragraph::new(form_lines).block(form_block).wrap(Wrap { trim: false });
    ...
    let cb_lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default().fg(pnl_color(risk.daily_pnl)).add_modifier(Modifier::BOLD),
            ),
        ]),
        Line::from(vec![
    ...
    let cb_block = Block::default()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(if risk.circuit_breaker_tripped { Color::Red } else { Color::Green }))
        .title(Span::styled(
            " CIRCUIT BREAKER ",
            Style::default().fg(if risk.circuit_breaker_tripped { Color::Red } else { Color::Green })
    ...
    for pos in positions {
        *exposure_by_market.entry(shorten_token(&pos.token_id, 14)).or_insert(0.0) += pos.usdc_spent;
    }
    ...
    let expo_lines: Vec<Line> = exposure_list.iter().map(|(tok, val)| {
        let bar_len = if max_exposure > 0.0 { (val / max_exposure * bar_max) as usize } else { 0 };
        ...
        Line::from(vec![
            ...
            Span::styled(format!(" ${:.0}", val), Style::default().fg(Color::DarkGray)),
        ])
    }).collect();
    ...
    let expo_para = Paragraph::new(expo_lines).block(expo_block).wrap(Wrap { trim: true });
...
fn render_performance_tab(
    ...
    let signal_line = match (perf.sig_avg_us, perf.sig_p99_us, perf.sig_count) {
        (Some(avg), Some(p99), count) => format!("  Signal latency: avg={}us  p99={}us  samples={}", avg, p99, count),
        _ => "  Signal latency: --".to_string(),
    };
    let fill_line = match (perf.fill_p50_us, perf.fill_p95_us, perf.fill_p99_us) {
        (Some(p50), Some(p95), Some(p99)) => format!("  Fill latency: p50={}us  p95={}us  p99={}us", p50, p95, p99),
    ...
    let top_lines = vec![
        Line::from(vec![
            ...
            Span::styled(quality.0, Style::default().fg(quality.1).add_modifier(Modifier::BOLD)),
            Span::styled(format!("  | msgs/s {:.0} (total {})", perf.msgs_per_sec, perf.msg_total), Style::default().fg(Color::DarkGray)),
        ]),
    ...
    f.render_widget(
        Paragraph::new(top_lines).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Cyan))
                .title(Span::styled(" PERFORMANCE OVERVIEW ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))),
        ),
        outer[0],
    );
    ...
    let middle = Layout::default()
        ...
        .constraints([Constraint::Percentage(50), Constraint::Percentage(50)])
        .split(outer[2]);
    render_kernel_telemetry(f, middle[0], kernel, kernel.available && (kernel.rtt.p99_us > RTT_ALERT_US || kernel.syscall.send_avg_us > SYSCALL_ALERT_US));

    let exec_lines = vec![
        Line::from(format!("  Trades: {}", state.execution_summary.trades)),
        Line::from(format!("  Fill rate: {:.1}%", state.execution_summary.fill_rate_pct)),
        Line::from(format!("  Reject rate: {:.1}%", state.execution_summary.reject_rate_pct)),
        Line::from(format!("  Avg slippage: {:.2} bps", state.execution_summary.avg_slippage_bps)),
        Line::from(format!("  Avg queue delay: {:.1} ms", state.execution_summary.avg_queue_delay_ms)),
        Line::from(format!("  Shadow realism gap: {:.2} bps", state.execution_summary.shadow_realism_gap_bps)),
    ];
    f.render_widget(
        Paragraph::new(exec_lines).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Magenta))
                .title(Span::styled(" EXECUTION KPI ", Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD))),
        ),
        middle[1],
    );
    ...
    let exp_lines = vec![
        Line::from(format!("  Variant A fills={} pnl={:+.2}", state.experiment_metrics.variant_a_fills, state.experiment_metrics.variant_a_realized_pnl)),
        Line::from(format!("  Variant B fills={} pnl={:+.2}", state.experiment_metrics.variant_b_fills, state.experiment_metrics.variant_b_realized_pnl)),
        Line::from(format!(
            "  Toggles: sizingB={} autoclaimB={} driftB={}",
            state.experiment_switches.sizing_variant_b,
            state.experiment_switches.autoclaim_variant_b,
    ...
    f.render_widget(
        Paragraph::new(exp_lines).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Green))
                .title(Span::styled(" A/B EXPERIMENTS ", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD))),
        ),
        bottom[0],
    );
    ...
    let win_rate = if snap.closed_trades.is_empty() {
        ...
        let wins = snap.closed_trades.iter().filter(|t| t.realized_pnl > 0.0).count() as f64;
    ...
    f.render_widget(
        Paragraph::new(health_lines).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Yellow))
                .title(Span::styled(" TRADING HEALTH ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))),
        ),
        bottom[1],
    );
}
...
fn render_twin_tab(f: &mut Frame, area: Rect, twin: &TwinUiSnapshot) {
    ...
    let status_color = if twin.enabled { Color::Green } else { Color::DarkGray };
    let nav_color = if twin.realized_pnl + twin.unrealized_pnl >= 0.0 { Color::Green } else { Color::Red };
    let header = vec![
        Line::from(vec![
            Span::styled(" Status: ", Style::default().fg(Color::DarkGray)),
            Span::styled(status_text, Style::default().fg(status_color).add_modifier(Modifier::BOLD)),
            Span::styled("   Generation: ", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("{}", twin.generation), Style::default().fg(Color::Cyan)),
        ]),
        Line::from(vec![
            Span::styled(" NAV: ", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("${:.2}", twin.nav), Style::default().fg(nav_color)),
            Span::styled("   Return: ", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("{:+.2}%", twin.nav_return_pct), Style::default().fg(nav_color)),
            Span::styled("   Realized: ", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("{:+.2}", twin.realized_pnl), Style::default().fg(nav_color)),
            Span::styled("   Unrealized: ", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("{:+.2}", twin.unrealized_pnl), Style::default().fg(nav_color)),
        ]),
    ];
    f.render_widget(
        Paragraph::new(header).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Cyan))
                .title(Span::styled(" BLINK TWIN STATUS ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD))),
        ),
        outer[0],
    );
    ...
    let cfg_lines = vec![
        ...
        Line::from(format!("  Slippage penalty: {:.2} bps", twin.slippage_penalty_bps)),
    ...
    f.render_widget(
        Paragraph::new(cfg_lines).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Magenta))
                .title(Span::styled(" TWIN CONFIG ", Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD))),
        ),
        middle[0],
    );
    ...
    f.render_widget(
        Paragraph::new(health_lines).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Yellow))
                .title(Span::styled(" TWIN HEALTH ", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD))),
        ),
        middle[1],
    );
    ...
    f.render_widget(
        Paragraph::new(risk_lines).block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Blue))
                .title(Span::styled(" TWIN PERFORMANCE ", Style::default().fg(Color::Blue).add_modifier(Modifier::BOLD))),
        ),
        perf[0],
    );

    let spark_data: Vec<u64> = if twin.equity_curve.len() >= 2 {
        let min_val = twin.equity_curve.iter().cloned().fold(f64::INFINITY, f64::min);
        let max_val = twin.equity_curve.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
        let range = (max_val - min_val).max(1e-9);
        twin.equity_curve
            .iter()
            .map(|&v| (((v - min_val) / range) * 100.0).round() as u64)
    ...
    let spark = Sparkline::default()
        ...
        .block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Green))
                .title(Span::styled(" TWIN EQUITY ", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD))),
        );
    f.render_widget(spark, perf[1]);

    let bottom_rows = vec![
        Row::new(vec![Cell::from("filled_orders"), Cell::from(twin.filled_orders.to_string())]),
        Row::new(vec![Cell::from("aborted_orders"), Cell::from(twin.aborted_orders.to_string())]),
        Row::new(vec![Cell::from("skipped_orders"), Cell::from(twin.skipped_orders.to_string())]),
    ];
    let bottom_table = Table::new(bottom_rows, [Constraint::Percentage(60), Constraint::Percentage(40)])
        .header(Row::new(vec![
            Cell::from("Metric").style(Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
        ...
        .block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Green))
                .title(Span::styled(" TWIN EXECUTION BREAKDOWN ", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD))),
        );
...
fn render_rejection_trend_overlay(
    ...
    let block = Block::default()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(Color::DarkGray))
        .title(Span::styled(" REJ 24H ", Style::default().fg(Color::DarkGray)));
...
fn render_experiment_switches(f: &mut Frame, area: Rect, _sel: usize, _editing: bool) {
    ...
    let block = Block::default()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(Color::Cyan))
        .title(Span::styled(" STRATEGY EXPERIMENTS ", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)));
    f.render_widget(Paragraph::new(lines).block(block), area);
}
...
fn render_notifications(f: &mut Frame, area: Rect, notifications: &[Notification]) {
    let active: Vec<&Notification> = notifications.iter()
    ...
    for (i, notif) in active.iter().enumerate() {
        ...
        if y + notif_height > area.height { break; }
        ...
        let block = Block::default()
            ...
            .title(Span::styled(
                format!(" {} [{remaining}s] ", match notif.kind {
...
fn render_hint(f: &mut Frame, area: Rect, active_tab: usize, search_active: bool, modern_theme: bool) {
    let global = vec![
        Span::styled("  [q] Quit  ", Style::default().fg(Color::DarkGray)),
        Span::styled("[p] Pause  ", Style::default().fg(Color::DarkGray)),
        Span::styled("[t] Theme  ", Style::default().fg(Color::DarkGray)),
        Span::styled("[1-6/Tab] Switch tab  ", Style::default().fg(Color::DarkGray)),
    ];
    ...
    let tab_hints = if search_active {
        ...
        match active_tab {
            TAB_DASHBOARD => vec![
                Span::styled("[Arrow/PgUp/PgDn] Scroll log", Style::default().fg(Color::DarkGray)),
            ],
            TAB_MARKETS => vec![
                Span::styled("[/] Search  ", Style::default().fg(Color::DarkGray)),
                Span::styled("[Arrow] Navigate  ", Style::default().fg(Color::DarkGray)),
                Span::styled("[Space] Toggle subscription", Style::default().fg(Color::DarkGray)),
            ],
            TAB_HISTORY => vec![
                Span::styled("[Arrow/PgUp/PgDn] Scroll", Style::default().fg(Color::DarkGray)),
            ],
            TAB_CONFIG => vec![
                ...
                Span::styled("  [x/c/v] A/B toggles", Style::default().fg(Color::DarkGray)),
            ],
            TAB_PERFORMANCE => vec![
                Span::styled("Live performance dashboards", Style::default().fg(Color::DarkGray)),
            ],
            TAB_TWIN => vec![
                Span::styled("Blink Twin live status + outcomes", Style::default().fg(Color::DarkGray)),
            ],
            _ => vec![],
        }
    };
    ...
    spans.push(Span::styled(
        if modern_theme { "Mode: Modern  " } else { "Mode: Native  " },
...
fn render_council_of_agents(f: &mut Frame, area: Rect) {
    ...
    let spans: Vec<Span> = agents.iter().map(|(name, _role, color)| {
    ...
    let block = Block::default()
        ...
        .title(Span::styled(" COUNCIL OF AGENTS ", Style::default().fg(MONO_PURPLE).add_modifier(Modifier::BOLD)));

    let para = Paragraph::new(Line::from(line_spans)).block(block).alignment(Alignment::Center);
