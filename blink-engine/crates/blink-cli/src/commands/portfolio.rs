...
async fn show_balances(ctx: CliContext) -> Result<()> {
    ...
    let pairs = [
        (
            "NAV (USDC)",
            data["nav_usdc"].as_f64().map(|v| format!("${:.4}", v)),
        ),
        (
            "Cash (USDC)",
            data["cash_usdc"].as_f64().map(|v| format!("${:.4}", v)),
        ),
        (
            "Invested (USDC)",
            data["invested_usdc"].as_f64().map(|v| format!("${:.4}", v)),
        ),
        (
            "Unrealised P&L",
            data["unrealized_pnl_usdc"].as_f64().map(format_pnl),
        ),
        (
            "Realised P&L",
            data["realized_pnl_usdc"].as_f64().map(format_pnl),
        ),
        (
            "Fees paid (USDC)",
            data["fees_paid_usdc"]
        ...
        (
            "Total signals",
            data["total_signals"].as_u64().map(|v| v.to_string()),
        ),
        (
            "Filled orders",
            data["filled_orders"].as_u64().map(|v| v.to_string()),
        ),
        (
            "Uptime (s)",
            data["uptime_secs"].as_u64().map(|v| v.to_string()),
        ),
    ];
...
async fn show_pnl(ctx: CliContext, limit: usize) -> Result<()> {
    let data = ctx
        .engine_get(&format!("/api/history?per_page={limit}&page=1"))
        .await
        .map_err(|e| anyhow::anyhow!("Engine unreachable — is Blink running? ({e})"))?;
    ...
    for t in trades.iter().take(limit) {
        let title = t["market_title"]
            .as_str()
            .or_else(|| t["token_id"].as_str())
            .unwrap_or("—");
